...
X = pd.concat([X[numeric_type], X[category_type].sub(X[category_type].min(), axis = 1)], axis = 1)
X = X[sorted(X.columns.values)]

test_X = pd.concat(
    [test_X[numeric_type], test_X[category_type].sub(test_X[category_type].min(), axis=1)], axis=1)
...
dump_svmlight_file(X, y, './data/train.libsvm', zero_based=True)

test_y = np.zeros(test_X.shape[0])

dump_svmlight_file(test_X, test_y, './data/test.libsvm', zero_based=True)